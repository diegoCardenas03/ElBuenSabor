...
export const ProductCards: React.FC<ProductCardsProps> = ({ products }) => {
  return (
    <div className="grid grid-cols-4 sm:grid-cols-3 md:grid-cols-4 gap-10">
      {products.map((product) => (
        <div
          key={product.id}
          className="bg-white border border-gray-300 rounded-lg p-4 flex flex-col items-center shadow hover:shadow-lg transition"
        >
          {/* Imagen del producto */}
          ...
          <button className="mt-2 bg-orange-400 hover:bg-orange-500 text-white px-4 py-1 rounded-full text-sm font-semibold">
            Añadir
          </button>
        </div>
      ))}
