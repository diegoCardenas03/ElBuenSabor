...
export const ProductFilterModal: FC<ProductFilterModalProps> = ({
  ...
  const [bestseller, setBestseller] = useState<boolean>(initialFilters.bestseller);
  ...
  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg p-6 w-full max-w-md shadow-lg relative">
        {/* Botón X para cerrar */}
        <button className="absolute top-3 right-3 text-gray-400 hover:text-gray-700 text-2xl font-bold focus:outline-none cursor-pointer" onClick={onClose} aria-label="Cerrar" type="button" > ×</button>
        ...
        <div className="flex flex-wrap gap-2 mb-6">
          {chips.map((chip) => (
            ...
            >
              {chip.label}
              {chip.active && (
                <button className="ml-2 font-bold focus:outline-none" onClick={e => {
                  e.stopPropagation();
                  chip.onRemove();
                }} type="button">×
                </button>
        ...
        <div className="flex justify-end gap-2">
          <button className="bg-gray-200 px-4 py-1 rounded hover:bg-gray-300" onClick={() => {
            setOrder('');
            setBestseller(false);
          }} type="button" > Limpiar
          </button>
          <button className="bg-[#FF9D3A] text-white px-4 py-1 rounded hover:bg-orange-500" onClick={handleApply} type="button" > Aplicar
          </button>
        </div>
      </div>
    </div>
